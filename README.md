...
8. `레시피 보기`를 눌러 설명, 필요 재료, 부족한 재료, 조리 단계를 확인합니다.
9. 바로 가능한 레시피의 `요리하기` 버튼을 누릅니다.
10. 확인 모달에서 레시피 이름, 차감될 재료, 현재 수량, 조리 후 수량을 확인합니다.
11. `확인하고 요리하기`를 눌러 조리를 확정합니다.
12. inventory 수량이 줄어들고 추천 목록이 다시 계산되는지 확인합니다.
13. 페이지를 새로고침한 뒤 변경된 inventory가 localStorage로 유지되는지 확인합니다.
14. `데모 재료 초기화` 버튼을 눌러 sampleIngredients로 복원되는지 확인합니다.
